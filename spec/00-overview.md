...
- **Signing preimage.** The preimage of every signature is `sha256(JCS(O))`, where `O` is the object with every member whose name is `sig` or begins with `sig_` removed. A single-signature object carries exactly one member, `sig`. A multi-signature object carries one `sig_<role>` member per required signer; every signer therefore signs identical bytes, and each signature MUST be verified against the key its role names. An object MUST NOT be treated as signed unless every signature its schema declares as required verifies. **Removal is top-level only.** A `sig` or `sig_<role>` member nested inside another member — a signed object carried as the payload of a signed message, say — stays in the outer preimage, so the outer signature covers the inner ones. The alternative reading, stripping recursively, would let a relay exchange an inner signature for another that verifies without disturbing the outer signature at all, and §6.3 asks that outer signature to be what catches exactly that.
