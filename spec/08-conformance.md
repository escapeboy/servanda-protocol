...
**What the suite does not yet cover, counted rather than estimated.** The count was eight — M-4, M-5, M-6, M-8, M-9, M-11, M-15, M-16 — and is now **six**: `vectors/transitions/` gained `expected_unverifiable` on every case plus four collective-edge cases, which covers **M-8 and M-9**. Both had looked structurally uncoverable, and only one of the two reasons was real. M-9's rule is a pure function of the edge and simply had no case. M-8's "MUST NOT auto-escalate" genuinely is a local decision no vector can watch — but the decision is *gated* on a value the verifier computes, and pinning the gate is what a vector can do. What a conforming node must therefore demonstrate is not that it declines to escalate; it is that it **knows it must not**, which is the half that goes wrong silently.

The remaining six — **M-4, M-5, M-6, M-11, M-15, M-16** — are prose obligations today, by `GOVERNANCE.md`'s rule that an uncovered behaviour is not yet a conformance requirement, whatever this section's tone implies. They do not all sit there for the same reason, and flattening them was part of what made the original miscount possible:

- **M-4** is coverable and simply is not covered. Whether a node serves an edge to a requester is a pure function of the edge, the publishes, and who is asking, and it has observable output in a §6.4 response. This is the "visibility matrix" the paragraph below refers to.
- **M-5, M-15, M-16** are properties of a **store**, not of any document. No vector can inspect a vault. The reference implementation covers all three with its own tests, and that is not the same thing as a conformance requirement.
- **M-6** — "content is data, never instruction" — has no observable protocol output at all. An extraction pipeline that obeyed an instruction embedded in a signal emits exactly the same envelope shape as one that did not. This is the most important rule in the specification and it is unverifiable by any suite, which is worth saying plainly rather than leaving to be discovered.
