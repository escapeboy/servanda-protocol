...
- A connector emits envelopes through an operation named `emit_envelope`. **How it delivers them is implementation-defined**, and the envelope schema is the only contract between connector and core; connector-specific fields go under `payload`.

  An earlier revision required connectors to be MCP servers. That was withdrawn because it contradicted the sentence beside it and because it inverts the direction the rest of the specification runs in: §7 makes the NODE an MCP server, so requiring the same of a connector would make the node a client of every connector it reads from, and would require each connector to be a long-lived process. Most are not — a git hook fires and exits, a mail poller runs on a timer, a library is called in-process by whatever already holds the signal. Naming a transport bought nothing the schema does not already guarantee, and it made a conforming implementation look non-conforming for a reason unrelated to what crosses the boundary. MCP remains one way to do it, and is the right one for a connector that genuinely is a server.
